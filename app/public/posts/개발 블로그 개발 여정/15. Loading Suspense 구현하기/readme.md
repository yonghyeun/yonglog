...
서버의 `RSCP` 를 클라이언트 단에서 `hydrate` 시키는 과정에서 `MDXRemote` 컴포넌트가 모든 렌더링 단계를 `block` 하기 떄문에 많은 시간의 지연이 발생한다.
...
이는 `MDXRemote` 컴포넌트 자체가 비동기적으로 처리 되며 `rehype-pretty-code` 의 플러그인들이 코드블록을 포맷팅 하는 동안 모든 렌더링 단계가 `block` 되기 떄문이다.
